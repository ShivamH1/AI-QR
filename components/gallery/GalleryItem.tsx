...
export default function GalleryItem({ entry }: GalleryItemProps) {
  ...
  return (
    ...
    >
      ...
      <div
        className={`relative w-full overflow-hidden ${isCard ? "aspect-[9/16]" : "aspect-[16/9]"}`}
      >
        ...
        <img
          ...
          alt={
            isCard
              ? `AI Digital ID for ${entry.cardDetails?.name}`
              : `AI QR code for ${entry.url}`
          }
      ...
      <div className="p-4 space-y-2">
        ...
        <div className="flex items-center gap-2">
          <span
            className={`w-1.5 h-1.5 rounded-full shrink-0 ${isCard ? "bg-brand-cyan" : "bg-brand-purple"}`}
          />
